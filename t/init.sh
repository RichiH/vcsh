...
test_expect_success 'VCSH_GITIGNORE variable is validated' \
	test_must_fail VCSH_GITIGNORE=x $VCSH init foo || false
	test_must_fail VCSH_GITIGNORE=nonsense $VCSH init foo || false
	test_must_fail VCSH_GITIGNORE=fhqwhgads $VCSH init foo || false
}
...
test_expect_success 'Init command does not set core.excludesfile with VCSH_GITIGNORE=none' \
	...
	test_must_fail $VCSH run test1 git config core.excludesfile || false
}
...
test_expect_success 'Init command does not set core.attributesfile with VCSH_GITATTRIBUTES=none' \
	...
	test_must_fail $VCSH run test1 git config core.attributesfile || false
}
EOF
